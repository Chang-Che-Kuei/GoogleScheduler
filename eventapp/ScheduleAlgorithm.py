...
class ScheduleAlgorithm:
	...
	def FindBlankBlock(self,timeRange,pref):
		...
		#print(start, end)
		#start = datetime.datetime(start[0], start[1], start[2], start[3], start[4]) + datetime.timedelta(hours=8)
		#end = datetime.datetime(end[0], end[1], end[2], end[3], end[4]) + datetime.timedelta(hours=8)
		#datetimeStart = datetime.date(start.year,start.month,start.day)
		#dayNum = (end - start).days + 1
		...
		#print('freeTime = ', freeTime, start[3], start[3]*60)
		# Get events from google calendar
		startD = self.GetUTCtimezone(start)
		endD   = self.GetUTCtimezone(end)
		events = self.rm.get_all_events(startD, endD)
	...
	def MakePreparationEventFormat(self,event,timeInfo):
		...
		gooEvent['summary'] = "Big_" + event['EventName'] + '_Preparation'
		...
		gooEvent['colorId'] = 11
		gooEvent['eventType'] = 1
		gooEvent['status'] = 0
		gooEvent['PreparingHours'] = event['PreparingTime']['PreparingHours']
		gooEvent['deadline'] = self.GetUTCtimezone(event['PreparingTime']['End'])
		print(gooEvent)
		return gooEvent

	def MakeFinalEventFormat(self,event):
		gooEvent = {}
		gooEvent['summary'] = "Big_" + event['EventName']
		if 'Description' in event:
			gooEvent['description'] = event['Description']
	...
	def AssignBlock(self,event,blankAndEvent,timeRange,pref,service,eventListHr):
		...
		for date, block in blank.items():
			for index in range(0,len(block),2):
				...
				if end-start < pref.minimumDuration and prepMin>= end-start:
					continue
				...
				if eventListHr == 0: # eventList() or editBigEvent()  don't need to add the info of prepRange and prepHr 
		...
		if 'FinalEvent' in event:
			...
			if event['FinalEvent']['Start'] != "":
				...
				if eventListHr == 0: # eventList() or editBigEvent()  don't need to add the info of prepRange and prepHr 
					finEvent['description'] += '\n' + strAppendDesciption + '\n' + strPrepHr
				print("I had insert fianl event")
				self.rm.insert_event(finEvent)
	...
	def TimeShift(self, pref, day_of_deleted_event):
		print('TimeShift ', day_of_deleted_event)
		...
		print(timeRange)
		blank, _ = self.FindBlankBlock(timeRange, pref)
		print(blank)
		...
		start = self.GetUTCtimezone(timeMin)
		end = self.GetUTCtimezone(timeMax)
		events = self.rm.get_all_events(start, end)
		...
		for i in range(len(events)):
			...
			#	print(events[i]['summary'])
				...
				for j in range(0, len(blank_block), 2):
					...
					if duration <= 0:
						self.rm.delete_event(events[i]['id'], flag=1)
